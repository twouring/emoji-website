...
const groups = {
  ...
  '社群、AI 與檔案': [
    ...
    ['S3_ENDPOINT','S3 Endpoint'],['S3_BUCKET','S3 Bucket'],['S3_REGION','S3 Region'],
...
function validate(key,input) {
  ...
  if(['ANTHROPIC_BASE_URL','S3_ENDPOINT','MEMBER_URL','ETHEREUM_RPC_URL'].includes(key)&&value&&!/^https?:\/\//.test(value))throw Object.assign(Error('網址必須以 http:// 或 https:// 開頭。'),{status:400});
  return value;
}
