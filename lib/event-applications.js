...
// 可申請空間（代碼 → 中文名）；前台表單、通知信與檔期 API 共用
const VENUES = { '3F': '三樓共享空間', '4F': '閣樓休憩空間', '2F': '二樓交誼廳', '1F': '一樓咖啡餐酒館' };
const venueLabel = code => VENUES[code] || VENUES['3F'];
/** 活動地點文字 → 佔用的空間代碼。
 *  ponytail: 由地點文字推斷；寫不出樓層（例如只填地址的全棟活動）視為佔用全部空間。要精確就替 events 加 venues 欄位。 */
function venuesOfLocation(location) {
  const text = String(location || ''), hit = [];
  for (const [code, re] of [['1F', /一樓|1\s*F|1\s*樓|咖啡餐酒館/i], ['2F', /二樓|2\s*F|2\s*樓|交誼廳/i], ['3F', /三樓|3\s*F|3\s*樓/i], ['4F', /閣樓|四樓|4\s*F|4\s*樓/i]])
    if (re.test(text)) hit.push(code);
  return hit.length ? hit : Object.keys(VENUES);
}
...
function normalizeEventApplication(body) {
  ...
  if (kind === 'community') value.venue = '3F';
    ...
    if (!Object.hasOwn(VENUES, body.venue)) return { error: '請選擇包場空間。' };
...
module.exports = { VENUES, venueLabel, venuesOfLocation, normalizeEventApplication, safeHttpUrl, applicationDeposit, PRIVATE_EVENT_DEPOSIT_TWD };
