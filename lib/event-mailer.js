...
async function deliverDue(q,send,origin='',unsubscribeSecret=''){
 ...
 try{
  ...
  else if(row.channel==='text')out=await providers.text({to:row.recipient,channel:row.provider_channel||'sms',body:row.subject+'\n'+row.body.slice(0,900)+'\n'+origin+'/events/'+encodeURIComponent(row.context.slug),statusCallback:origin+'/api/twilio/events/status'});
  else if(row.channel==='push')out=await providers.push(row.push_subscription,{title:row.subject,body:row.body.slice(0,240),url:'/events/'+encodeURIComponent(row.context.slug)});
...
async function queueMemberChannels(q,{id,eventId,email,subject,body,category}){const channels=require('./event-channels'),text=channels.twilioConfig(),push=channels.pushConfig();if(!text&&!push)return;const defaults={email:true,text:true,push:true},member=(await q(`SELECT u.phone,u.phone_verified_at IS NOT NULL AS phone_verified,u.message_channel,COALESCE(p.channels->$3,$4::jsonb) AS channels,COALESCE((SELECT jsonb_agg(s.subscription) FROM event_push_subscriptions s WHERE s.user_id=u.id),'[]'::jsonb) AS subscriptions FROM users u LEFT JOIN event_notification_preferences p ON p.event_id=$2 AND p.user_id=u.id WHERE lower(u.email)=lower($1)`,[email,eventId,category,JSON.stringify(defaults)])).rows[0];if(!member)return;if(text&&member.phone_verified&&member.channels?.text!==false)await q(`INSERT INTO event_deliveries(id,message_id,email,recipient,channel,provider_channel,subject,body,next_attempt_at) VALUES($1,$2,$3,$4,'text',$5,$6,$7,now()) ON CONFLICT DO NOTHING`,[id+'_text',id,email,member.phone,member.message_channel,subject,body]);if(push&&member.channels?.push!==false)for(const subscription of member.subscriptions)await q(`INSERT INTO event_deliveries(id,message_id,email,recipient,channel,subject,body,next_attempt_at,push_subscription) VALUES($1,$2,$3,$4,'push',$5,$6,now(),$7) ON CONFLICT DO NOTHING`,[id+'_push_'+require('node:crypto').createHash('sha256').update(subscription.endpoint).digest('hex').slice(0,12),id,email,subscription.endpoint,subject,body,JSON.stringify(subscription)]);}
async function queueEventInvitation(q,{event,registrationId,email,name,language='zh',origin=''}){
 ...
 return queueEventMail(q,{id:'invite_'+registrationId,eventId:event.id,registrationId,ticketVersion:1,expectedStatus:'invited',email,subject:copy[0]+' · '+title,body:name+'\n\n'+copy[1]+'\n'+title+'\n\n'+origin+(lang==='zh'?'':'/'+lang)+'/events/'+encodeURIComponent(event.slug)});
}
