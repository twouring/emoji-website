...
async function queueMemberChannels(q,{id,eventId,email,subject,body,category,only}){const channels=require('./event-channels'),text=channels.twilioConfig(),push=channels.pushConfig();if(!text&&!push)return 0;const allowed=new Set(only||['text','push']),defaults={email:true,text:true,push:true},member=(await q(`SELECT u.phone,u.phone_verified_at IS NOT NULL AS phone_verified,u.message_channel,COALESCE(p.channels->$3,$4::jsonb) AS channels,COALESCE((SELECT jsonb_agg(s.subscription) FROM event_push_subscriptions s WHERE s.user_id=u.id),'[]'::jsonb) AS subscriptions FROM users u LEFT JOIN event_notification_preferences p ON p.event_id=$2 AND p.user_id=u.id WHERE lower(u.email)=lower($1)`,[email,eventId,category,JSON.stringify(defaults)])).rows[0];if(!member)return 0;let count=0;if(allowed.has('text')&&text&&member.phone_verified&&member.channels?.text!==false){const result=await q(`INSERT INTO event_deliveries(id,message_id,email,recipient,channel,provider_channel,subject,body,next_attempt_at) VALUES($1,$2,$3,$4,'text',$5,$6,$7,now()) ON CONFLICT DO NOTHING`,[id+'_text',id,email,member.phone,member.message_channel,subject,body]);count+=result.rowCount??result.rows?.length??0;}if(allowed.has('push')&&push&&member.channels?.push!==false)for(const subscription of member.subscriptions){const result=await q(`INSERT INTO event_deliveries(id,message_id,email,recipient,channel,subject,body,next_attempt_at,push_subscription) VALUES($1,$2,$3,$4,'push',$5,$6,now(),$7) ON CONFLICT DO NOTHING`,[id+'_push_'+require('node:crypto').createHash('sha256').update(subscription.endpoint).digest('hex').slice(0,12),id,email,subscription.endpoint,subject,body,JSON.stringify(subscription)]);count+=result.rowCount??result.rows?.length??0;}return count;}
...
const eventValue=(event,lang,key)=>event?.translations?.[lang]?.[key]||event?.[key]||'';
const eventTime=value=>value&&Number.isFinite(+new Date(value))?+new Date(value):null;
function criticalEventChanges(previous,event,lang='zh'){
 const changes=[];
 if(eventValue(previous,lang,'title')!==eventValue(event,lang,'title'))changes.push('title');
 if(eventTime(previous?.starts_at)!==eventTime(event?.starts_at)||eventTime(previous?.ends_at)!==eventTime(event?.ends_at))changes.push('time');
 if(eventValue(previous,lang,'location')!==eventValue(event,lang,'location'))changes.push('location');
 return changes;
}
async function queueEventUpdate(q,{previous,event,origin='',emailEnabled=true,updateId=''}){
 const allChanges=[...new Set(['zh','en','ja'].flatMap(lang=>criticalEventChanges(previous,event,lang)))];
 if(!allChanges.length)return {changes:[],recipients:0,deliveries:0};
 const recipients=(await q(`SELECT DISTINCT ON (email) email,language,registration_id,ticket_version FROM (
  SELECT lower(u.email) AS email,r.language,r.id AS registration_id,r.ticket_version,0 AS priority FROM event_regs r JOIN users u ON u.id=r.user_id WHERE r.event_id=$1 AND r.status='registered'
  UNION ALL SELECT lower(a.email),r.language,r.id,r.ticket_version,1 FROM event_attendees a JOIN event_regs r ON r.id=a.registration_id LEFT JOIN event_ticket_orders o ON o.id=a.order_id WHERE r.event_id=$1 AND r.status='registered' AND (a.order_id IS NULL OR o.status='registered')
 ) recipients WHERE email<>'' ORDER BY email,priority`,[event.id])).rows;
 const pushEnabled=!!require('./event-channels').pushConfig(),hash=require('node:crypto').createHash('sha256').update(JSON.stringify([updateId,event.id,event.title,event.starts_at,event.ends_at,event.location,event.translations])).digest('hex').slice(0,16);
 let notified=0,deliveries=0;
 for(const recipient of recipients){const lang=['en','ja'].includes(recipient.language)?recipient.language:'zh',changes=criticalEventChanges(previous,event,lang);if(!changes.length)continue;notified++;if(!emailEnabled&&!pushEnabled)continue;
  const copy={zh:{subject:'活動資訊已更新',intro:'以下活動資訊已更新',title:'活動名稱',time:'時間',location:'地點'},en:{subject:'Event details updated',intro:'The following event details have changed',title:'Event name',time:'Date or time',location:'Location'},ja:{subject:'イベント情報が更新されました',intro:'次のイベント情報が更新されました',title:'イベント名',time:'日時',location:'場所'}}[lang],title=eventValue(event,lang,'title'),locale=lang==='zh'?'zh-TW':lang==='ja'?'ja-JP':'en-US',start=event.starts_at?new Date(event.starts_at).toLocaleString(locale,{timeZone:'Asia/Taipei'}):'',end=event.ends_at?new Date(event.ends_at).toLocaleString(locale,{timeZone:'Asia/Taipei'}):'',url=origin+(lang==='zh'?'':'/'+lang)+'/events/'+encodeURIComponent(event.slug),body=[copy.intro+'：'+changes.map(key=>copy[key]).join('、'),title,start+(end?' – '+end:'')+' (UTC+8)',eventValue(event,lang,'location'),url].filter(Boolean).join('\n\n'),id='event_update_'+hash+'_'+require('node:crypto').createHash('sha256').update(recipient.registration_id+':'+recipient.email).digest('hex').slice(0,16),attachments=event.starts_at?[{filename:'event.ics',content:Buffer.from(require('./event-calendar').eventCalendar({...event,title,description:eventValue(event,lang,'description'),location:eventValue(event,lang,'location')},origin)).toString('base64')}]:[];
  const mail=await queueEventMail(q,{id,eventId:event.id,registrationId:recipient.registration_id,ticketVersion:recipient.ticket_version,expectedStatus:'registered',email:recipient.email,subject:copy.subject+' · '+title,body,attachments,emailEnabled}),mailCount=mail.rowCount??mail.rows?.length??0;deliveries+=mailCount;
  const pushCount=pushEnabled?await queueMemberChannels(q,{id,eventId:event.id,email:recipient.email,subject:copy.subject+' · '+title,body,category:'updates',only:['push']}):0;deliveries+=pushCount;if(!mailCount&&!pushCount)await q('DELETE FROM event_messages WHERE id=$1',[id]);
 }
 return {changes:allChanges,recipients:notified,deliveries};
}
...
module.exports.criticalEventChanges=criticalEventChanges;
module.exports.queueEventUpdate=queueEventUpdate;
