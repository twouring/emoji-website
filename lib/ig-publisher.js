...
async function getToken(deps) {
  const row = (await deps.q(`SELECT value FROM system_settings WHERE key=$1`, [TOKEN_KEY])).rows[0];
  if (row) return systemSettings.decrypt(TOKEN_KEY,row.value,process.env.APP_SECRET);
  const legacy = (await deps.q(`SELECT value FROM site_content WHERE key='ig_access_token'`)).rows[0];
  if (legacy?.value) { await saveToken(deps,legacy.value);await deps.q(`DELETE FROM site_content WHERE key='ig_access_token'`);return legacy.value; }
  return process.env.IG_ACCESS_TOKEN || '';
}

async function saveToken(deps, token) {
  await deps.q(
    `INSERT INTO system_settings (key,value,updated_by,updated_at) VALUES ($1,$2,'ig-token-refresh',now())
     ON CONFLICT (key) DO UPDATE SET value=EXCLUDED.value,updated_by=EXCLUDED.updated_by,updated_at=now()`,
    [TOKEN_KEY, systemSettings.encrypt(TOKEN_KEY,token,process.env.APP_SECRET)],
  );
}
...
// 物件儲存不公開（Railway bucket／MinIO 皆同），對外一律走同網域 /ig-media/<key> 代理；DB 內存的網址即此形式
const publicUrl = key => `${String(process.env.PUBLIC_ORIGIN || 'https://www.emoji.tw').replace(/\/$/, '')}/ig-media/${key}`;
function s3Client() {
...
// 讀出物件供 /ig-media/<key> 代理；不存在回 null
async function getAsset(key) {
  if (!s3Enabled()) return null;
  const mc = s3Client();
  try {
    const { GetObjectCommand } = require('@aws-sdk/client-s3');
    const r = await mc.send(new GetObjectCommand({ Bucket: S3_BUCKET(), Key: key }));
    return { body: r.Body, contentType: r.ContentType || 'application/octet-stream', contentLength: r.ContentLength, disposition: r.ContentDisposition || '' };
  } catch (e) {
    if (e.name === 'NoSuchKey' || e.$metadata?.httpStatusCode === 404) return null;
    throw e;
  } finally { /* body stream 仍需 client；由呼叫端讀完後 GC */ }
}

// 列舉 MinIO 物件（找未登記的原始素材用）；回 [{key,size,lastModified,url}]
async function listAssets(prefix = 'assets/', max = 1000) {
  if (!s3Enabled()) return [];
  const mc = s3Client();
  try {
    ...
    const out = []; let token;
    do {
      const r = await mc.send(new ListObjectsV2Command({ Bucket: S3_BUCKET(), Prefix: prefix, ContinuationToken: token, MaxKeys: Math.min(max - out.length, 1000) }));
      for (const o of r.Contents || []) out.push({ key: o.Key, size: o.Size, lastModified: o.LastModified, url: publicUrl(o.Key) });
  ...
  try {
    ...
    for (let i = 0; i < post.pages.length; i++) {
      ...
      if (useS3) {
        ...
        files.push(publicUrl(`posts/${file}`));
      } else {
...
module.exports = { s3Enabled, uploadAsset, getAsset, publicUrl, listAssets, normalizePage, checkBanned, checkStyle, renderPostImages, publishPost, publishDue, refreshToken, getToken, bannedList, aiPhraseList };
