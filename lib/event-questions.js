'use strict';
const {richText}=require('./event-rich-text');
...
function questions(input){
  ...
  return input.map(q=>{
    ...
    if(q.type==='terms'&&q.terms!==undefined){
      const terms=q.terms,kind=terms.kind||'text';
      if(!['text','link'].includes(kind)||typeof terms.show_before_accept!=='boolean'||typeof terms.require_signature!=='boolean')throw new Error('條款需設定內容類型、閱讀方式與簽名要求。');
      const normalized={kind,show_before_accept:terms.show_before_accept,require_signature:terms.require_signature};
      if(kind==='link'){
        let url;try{url=new URL(terms.url);}catch{}
        if(!url||url.protocol!=='https:'||url.username||url.password||url.port)throw new Error('條款連結需為有效的 HTTPS 網址。');
        normalized.url=url.href;
      }else{
        const content=terms.content||{};
        normalized.content={};
        for(const lang of ['zh','en','ja']){
          if(typeof content[lang]!=='string')throw new Error('條款內容格式不正確。');
          normalized.content[lang]=richText(content[lang]);
        }
        if(!normalized.content.zh.replace(/<[^>]+>/g,'').trim())throw new Error('請填寫中文條款內容。');
      }
      out.terms=normalized;
    }
    out.translations={};
...
function answers(list,input={}){
  ...
  return list.map(q=>{
    ...
    if(q.type==='terms'&&q.terms){
      if(value===undefined)value={accepted:false,reviewed:false,signature:''};
      if(!value||typeof value!=='object'||Array.isArray(value)||typeof value.accepted!=='boolean'||typeof value.reviewed!=='boolean'||typeof value.signature!=='string'||value.signature.length>200)throw new Error(q.label+'：條款同意格式不正確。');
      value={accepted:value.accepted,reviewed:value.reviewed,signature:value.signature.trim()};
      if(value.accepted&&q.terms.show_before_accept&&!value.reviewed)throw new Error(q.label+'：請先閱讀條款內容。');
      if(value.accepted&&q.terms.require_signature&&!value.signature)throw new Error(q.label+'：請輸入簽名。');
    }else if(['checkbox','terms'].includes(q.type)){
    ...
    if(q.required&&(!value||(Array.isArray(value)&&!value.length)||(q.type==='terms'&&q.terms&&!value.accepted)))throw new Error(q.label+'：此題必填。');
