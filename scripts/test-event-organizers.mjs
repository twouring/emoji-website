...
test('organizer isolation, scoped cohosts, translations, publication and registration persist in PostgreSQL',{skip:!dbUrl,timeout:60000},async()=>{
 ...
 try{
  ...
  const oldEmbed=await fetch(origin+'/en/embed/events/'+oldFirstSlug+'?source=old-link',{redirect:'manual'});assert.equal(oldEmbed.status,302);assert.equal(oldEmbed.headers.get('location'),'/en/embed/events/'+first.slug+'?source=old-link');
  ...
  await call('/admin/events/'+other.id+'/integration/key',{as:a,method:'POST',status:404});
  const firstKey=await call('/admin/events/'+first.id+'/integration/key',{as:a,method:'POST',status:201});assert.match(firstKey.key,/^evk_[A-Za-z0-9_-]{43}$/);
  assert.equal((await call('/integrations/events/'+first.id,{as:firstKey.key})).event.id,first.id);
  await call('/integrations/events/'+other.id,{as:firstKey.key,status:401});
  const nextKey=await call('/admin/events/'+first.id+'/integration/key',{as:a,method:'POST',status:201});assert.notEqual(nextKey.key,firstKey.key);
  await call('/integrations/events/'+first.id,{as:firstKey.key,status:401});
  assert.equal((await call('/integrations/events/'+first.id+'/check-in',{as:nextKey.key})).guests.length,1);
  await call('/integrations/events/'+first.id+'/check-in/lookup?token='+encodeURIComponent(regs.regs[0].attendees[0].checkin_token),{as:nextKey.key});
  await call('/integrations/events/'+first.id+'/check-in',{as:nextKey.key,method:'POST',body:{token:regs.regs[0].attendees[0].checkin_token}});
  await call('/integrations/events/'+first.id+'/check-in/'+regs.regs[0].id,{as:nextKey.key,method:'DELETE'});
  await call('/admin/events/'+first.id+'/integration/key',{as:a,method:'DELETE'});
  await call('/integrations/events/'+first.id,{as:nextKey.key,status:401});
  ...
  const criticalUpdate=await call('/admin/events',{as:a,method:'POST',body:{...payload,id:first.id,slug:first.slug,location:'Updated Taipei'}});assert.deepEqual(criticalUpdate.critical_changes,['location']);assert.equal(criticalUpdate.notification_recipients,1);assert.equal(criticalUpdate.notification_deliveries,0);assert.match(criticalUpdate.notice,/通知服務尚未設定/);assert.equal(Number((await pool.query("SELECT COUNT(*) AS n FROM event_messages WHERE id LIKE 'event_update_%'")).rows[0].n),0);
