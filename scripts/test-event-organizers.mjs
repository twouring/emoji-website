...
test('organizer isolation, scoped cohosts, translations, publication and registration persist in PostgreSQL',{skip:!dbUrl,timeout:60000},async()=>{
 ...
 try{
  ...
  const meeting=await call('/admin/events/'+first.id+'/meeting',{as:a});assert.equal(meeting.google_meet.configured,false);assert.equal(meeting.google_meet.connected,false);
  await call('/admin/events/'+first.id+'/meeting/connect',{as:a,method:'POST',body:{redirect:'/organizer/events/'+first.id},status:503});await call('/admin/events/'+first.id+'/meeting',{as:b,status:404});
