...
test('critical event updates notify only affected registered languages and attach the new calendar',async()=>{
 const {criticalEventChanges,queueEventUpdate}=createRequire(import.meta.url)('../lib/event-mailer'),previous={id:'event_1',slug:'demo',title:'中文活動',description:'內容',location:'台北',starts_at:'2026-10-31T10:00:00Z',ends_at:'2026-10-31T12:00:00Z',status:'報名中',translations:{en:{title:'English event',location:'Taipei'},ja:{title:'日本語イベント',location:'台北'}}},event={...previous,translations:{...previous.translations,en:{...previous.translations.en,title:'Updated event'}}};
 assert.deepEqual(criticalEventChanges(previous,event,'en'),['title']);assert.deepEqual(criticalEventChanges(previous,event,'ja'),[]);assert.deepEqual(criticalEventChanges(previous,{...event,description:'new copy'},'ja'),[]);
 const writes=[],q=async(sql,args)=>{if(sql.includes('SELECT DISTINCT ON (email)'))return {rows:[{email:'en@example.test',language:'en',registration_id:'reg_en',ticket_version:2},{email:'ja@example.test',language:'ja',registration_id:'reg_ja',ticket_version:1}]};writes.push({sql,args});return {rows:[{id:args[0]}],rowCount:1};};
 const result=await queueEventUpdate(q,{previous,event,origin:'https://emoji.test',emailEnabled:true,updateId:'activity_1'});assert.deepEqual(result,{changes:['title'],recipients:1,deliveries:1});assert.equal(writes.length,1);assert.match(writes[0].args[3],/Event details updated/);assert.match(writes[0].args[4],/Updated event/);const attachment=JSON.parse(writes[0].args[5])[0],firstId=writes[0].args[0];assert.equal(attachment.filename,'event.ics');assert.match(Buffer.from(attachment.content,'base64').toString(),/SUMMARY:Updated event/);
 writes.length=0;await queueEventUpdate(q,{previous,event,origin:'https://emoji.test',emailEnabled:true,updateId:'activity_2'});assert.notEqual(writes[0].args[0],firstId);
 let queried=false;assert.deepEqual(await queueEventUpdate(async()=>{queried=true;return {rows:[]};},{previous,event:{...previous,description:'Only description changed'},origin:'https://emoji.test'}),{changes:[],recipients:0,deliveries:0});assert.equal(queried,false);
});
